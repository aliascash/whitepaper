...
The Spectrecoin blockchain is based on the design from Bitcoin Core (except the consensus mechanisms) and in both systems there are ... possible receiving addresses. This is an extremely large number and it would give every person on Earth ... different receiving addresses to use. The fact that it is possible to re-use an address more than once can be considerd a fluke and is not by design.
